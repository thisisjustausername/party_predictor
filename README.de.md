...
4. Lade das BERT-Modell von HuggingFace herunter.<br/>
  Optional: Garantiere einen schnelleren Download indem Du zuerst `hf auth login` in deinem Terminal ausführst, um dich bei HuggingFace anzumelden.
   ```bash
   python3 -m bert.download_bert
   ```

5. Viel Spaß mit dem Code. Beachte, dass alle Dateien mit absoluten Imports geschrieben wurden und daher aus dem Root-Verzeichnis des Repositories gestartet werden müssen.<br/>
   ```bash
